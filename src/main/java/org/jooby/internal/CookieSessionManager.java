 ...
 * @author edgar
 */
public class CookieSessionManager implements org.jooby.internal.SessionManager {
  ...
  public Session create(final Request req, final Response rsp) {
    Session session = new org.jooby.internal.SessionImpl.Builder(resolver, true, Session.COOKIE_SESSION, -1).build();
  ...
  public Session get(final Request req, final Response rsp) {
    return req.cookie(cookie.name().get()).toOptional().map(raw -> {
      org.jooby.internal.SessionImpl.Builder session = new SessionImpl.Builder(resolver, false, Session.COOKIE_SESSION,
                                                                               -1);
    ...
    return (req, rsp, result) -> {
      req.ifSession().ifPresent(session -> {
        ...
        if (dirty) {
          ...
          rsp.cookie(new Definition(cookie).value(signed));
        } else if (timeout > 0) {
          // touch session
          value.ifPresent(raw -> rsp.cookie(new Definition(cookie).value(raw)));
        }
      });
      return result;
    };
