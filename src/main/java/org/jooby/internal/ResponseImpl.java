...
public class ResponseImpl implements Response {
  ...
  private List<Complete> complete = new ArrayList<>();

  private org.jooby.internal.RequestImpl req;
  ...
  public Response clearCookie(final String name) {
    ...
    return cookie(new Definition(name).maxAge(0));
  }
  ...
  public void send(final Result result) throws Throwable {
    ...
    if (value != null) {
      ...
      org.jooby.internal.HttpRendererContext ctx = new HttpRendererContext(
          renderers,
          rsp,
          setLen,
          setType,
    ...
    if (!committed()) {
      if (value instanceof Iterable) {
        List<String> values = StreamSupport.stream(((Iterable<Object>) value).spliterator(), false)
            .map(org.jooby.internal.Headers::encode)
            .collect(Collectors.toList());
