...
public class OptionsHandlerTest {
  ...
  private Block next = unit -> {
    Chain chain = unit.get(Chain.class);
    chain.next(unit.get(Request.class), unit.get(Response.class));
  };

  @Test
  public void handle() throws Exception {
    new MockUnit(Request.class, Response.class, Chain.class, Definition.class)
        .expect(next)
        .expect(allow(false))
        .expect(path)
        .expect(method("GET"))
        ...
        .run(unit -> {
          Set<Definition> routes = Sets.newHashSet(unit.get(Definition.class));
          new OptionsHandler(routes)
              .handle(unit.get(Request.class), unit.get(Response.class),
                  unit.get(Chain.class));
        });
  }
  ...
  private Block matches(final String method, final boolean matches) {
    return unit -> {
      ...
      Definition def = unit.get(Definition.class);
  ...
  private Block routeMethod(final String method) {
    return unit -> {
      Definition req = unit.get(Definition.class);
      expect(req.method()).andReturn(method);
    };
  }
